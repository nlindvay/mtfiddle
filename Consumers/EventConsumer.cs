...
public class EventConsumer : IConsumer<ClaimCheck>
{
    ...
    public EventConsumer(ClaimProvider claimCheckProvider, ILogger<IConsumer<ClaimCheck>> logger)
    {
    ...
    public async Task Consume(ConsumeContext<ClaimCheck> context)
    {
        ...
        _logger.LogInformation("EventConsumer: {Type} {Content}", context.Headers.Get<string>("type"), content);
    }
}